...
// 调试变量（已注释，需要时取消注释）
// volatile uint16_t g_last_can_std_id = 0;  // 最近一次接收的标准ID（用于调试上送）
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    // 调试代码（已注释，需要时取消注释）
    // g_last_can_std_id = (uint16_t)RxHeader.StdId;  // 记录最近一次的 StdId
	if(hcan == &hcan1)
		CAN1_Receive(RxHeader.StdId, CanRxData_FIFO0);   //进行CAN数据解析
	// 如需在 CAN2 上接入电机，可取消下行注释
	// else if(hcan == &hcan2)
	// 	CAN1_Receive(RxHeader.StdId, CanRxData_FIFO0);
}
